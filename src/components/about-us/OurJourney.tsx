...
function StatIcon() {
  return <Home className="mb-3 h-5 w-5 text-violet-400 md:h-6 md:w-6" aria-hidden="true" />;
}
      ...
      <div className="mx-auto grid max-w-[1920px] items-center gap-8 px-4 py-12 md:grid-cols-2 md:gap-6 md:px-6 md:py-16 lg:px-8 lg:py-20 xl:px-12">
        <div className="order-2 flex flex-col md:order-1">
          ...
          <div className="mt-8 grid grid-cols-2 gap-3 md:mt-10 md:grid-cols-3 lg:gap-4">
            {STATS.map((stat) => (
              <div
                key={stat.label}
                data-testid={`our-journey-stat-${stat.label.toLowerCase().replace(/\s+/g, "-")}`}
        ...
        <div className="relative order-1 md:order-2">
          <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl md:aspect-square">
            <Image
              src={JOURNEY_IMAGE}
              alt="Hand holding a miniature modern house architectural model"
              fill
